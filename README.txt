...
The Node and Express backend uses the parser written in C to parse the GPX files and convert them into a JavaScript readable JSON format. The backend stores and retrieves this data from the MySQL database and serves it to the frontend. The user can also create and modify the GPX files from the user interface.
